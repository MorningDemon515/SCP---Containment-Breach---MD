;SCP - Containment Breach Morning Demon
...
Global VersionNumber$ = "6.0"
Global GameVersionNumber$ = "1.3.11"
Global CompatibleNumber$ = "6.0" ;Only change this if the version given isn't working with the current build version - ENDSHN
...
Function MouseLook()
	...
	If KillTimer >= 0 And FallTimer >=0 Then
		...
		;kĂ¤Ă¤nnetĂ¤Ă¤n kameraa sivulle jos pelaaja on vammautunut
							...
							Else
								For i = 2 To 6
									If KeyHit(i) Then
										...
										;jos nykyistĂ¤ kanavaa ollaan soitettu, laitetaan jatketaan toistoa samasta kohdasta
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			...
			Select OptionsMenu
				...
				Case 4 ;Advanced
					...
					;TextEnable% = DrawTick(x + 270 * MenuScale, y + MenuScale, TextEnable%)
					;If TextEnable_Prev% <> TextEnable
...
Function UpdateMTF%()
	...
	;mtf ei vielĂ¤ spawnannut, spawnataan jos pelaaja menee tarpeeksi lĂ¤helle gate b:tĂ¤
	If MTFtimer = 0 Then
...
Function CircleToLineSegIsect% (cx#, cy#, r#, l1x#, l1y#, l2x#, l2y#)
	...
	;Palauttaa:
	;  True (1) kun:
	;      YmpyrĂ¤ [keskipiste = (cx, cy): sĂ¤de = r]
	;      leikkaa janan, joka kulkee pisteiden (l1x, l1y) & (l2x, l2y) kaitta
	;  False (0) muulloin
	
	;YmpyrĂ¤n keskipisteen ja (ainakin toisen) janan pĂ¤Ă¤tepisteen etĂ¤isyys < r
	;-> leikkaus
	...
	;Vektorit (janan vektori ja vektorit janan pĂ¤Ă¤tepisteistĂ¤ ympyrĂ¤n keskipisteeseen)
	...
	;Janan pĂ¤Ă¤tepisteiden kautta kulkevan suoran ;yhtĂ¤lĂ¶; (ax + by + c = 0)
	Local a# = (l2y - l1y) / (l2x - l1x)
	Local b# = -1
	Local c# = -(l2y - l1y) / (l2x - l1x) * l1x + l1y
	
	;YmpyrĂ¤n keskipisteen etĂ¤isyys suorasta
	Local d# = Abs(a * cx + b * cy + c) / Sqr(a * a + b * b)
	
	;YmpyrĂ¤ on liian kaukana
	;-> ei leikkausta
	If d > r Then Return False
	
	;Local kateetin_pituus# = Cos(angle) * hyp
	
	;Jos pĂ¤Ă¤stĂ¤Ă¤n tĂ¤nne saakka, ympyrĂ¤ ja jana leikkaavat (tai ovat sisĂ¤kkĂ¤in)
	Return True
End Function
...
Function CatchErrors(location$)
	...
	If Len(errStr)>0 Then
		If FileType(ErrorFile)=0 Then
			...
			WriteLine errF,"An error occured in SCP - Containment Breach MD!"
