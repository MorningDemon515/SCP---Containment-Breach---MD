...
WinHandle=CreateWindow("SCP-CB MD Map Creator "+versionnumber,GraphicsWidth()/2-ResWidth/2, GraphicsHeight()/2-ResHeight/2,ResWidth,ResHeight,0, 13) 
	...
	If FileType("CONFIG_TO2D.SI")=1
		...
		If CurrMapGrid%=0
			...
			If (Not hasEvent)
				...
				DisableGadget event_prob
				GridGadgetText="Name: "+Map(Grid_SelectedX,Grid_SelectedY)\Name+Chr(13)+"Angle: "+MapAngle(Grid_SelectedX,Grid_SelectedY)+"бу"
			Else
				...
				If MapEvent(Grid_SelectedX,Grid_SelectedY)<>"" And MapEvent(Grid_SelectedX,Grid_SelectedY)<>"[none]"
					...
					GridGadgetText="Name: "+Map(Grid_SelectedX,Grid_SelectedY)\Name+Chr(13)+"Angle: "+MapAngle(Grid_SelectedX,Grid_SelectedY)+"бу"+Chr(13)+"Event: "+MapEvent(Grid_SelectedX,Grid_SelectedY)+Chr(13)+"Event Chance: "+Int(MapEventProb(Grid_SelectedX,Grid_SelectedY)*100)+"%"
				Else
					SetGadgetText event_prob_label, ""
					SetSliderValue event_prob,99
					DisableGadget event_prob
		...
		ElseIf CurrMapGrid%=1
			...
			SetSliderValue event_prob,99
			DisableGadget event_prob
			GridGadgetText="Name: "+ForestPlace(Grid_SelectedX,Grid_SelectedY)\Name+Chr(13)+"Angle: "+ForestPlaceAngle(Grid_SelectedX,Grid_SelectedY)+"бу"
		Else
			For rt.RoomTemplates = Each RoomTemplates
			...
			SetSliderValue event_prob,99
			DisableGadget event_prob
			GridGadgetText="Name: "+MTRoom(Grid_SelectedX,Grid_SelectedY)\Name+Chr(13)+"Angle: "+MTRoomAngle(Grid_SelectedX,Grid_SelectedY)+"бу"
		EndIf
					...
					Else
						...
						If Grid_SelectedX=x And Grid_SelectedY=y
							If PrevSelectedX<>Grid_SelectedX Or PrevSelectedY<>Grid_SelectedY
								ChangeGridGadget = True
								If MapEvent(x,y)<>"" And MapEvent(x,y)<>"[none]"
									GridGadgetText = "Name: "+Map(x,y)\Name+Chr(13)+"Angle: "+MapAngle(x,y)+"бу"+Chr(13)+"Event: "+MapEvent(x,y)+Chr(13)+"Event Chance: "+Int(MapEventProb(x,y)*100)+"%"
									SetSliderValue(event_prob,Int(MapEventProb(x,y)*100)-1)
								Else
									GridGadgetText = "Name: "+Map(x,y)\Name+Chr(13)+"Angle: "+MapAngle(x,y)+"бу"
									;SetSliderValue(event_prob,99)
								EndIf
								If GadgetText(event_prob_label)<>""
				...
				For y = 0 To ForestGridSize
					...
					Else
						...
						If Grid_SelectedX=x And Grid_SelectedY=y
							If PrevSelectedX<>Grid_SelectedX Or PrevSelectedY<>Grid_SelectedY
								ChangeGridGadget = True
								GridGadgetText = "Name: "+ForestPlace(x,y)\Name+Chr(13)+"Angle: "+ForestPlaceAngle(x,y)+"бу"
							EndIf
						EndIf
					EndIf
				Next
				...
				For y = 0 To MT_GridSize
					...
					Else
						...
						If Grid_SelectedX=x And Grid_SelectedY=y
							If PrevSelectedX<>Grid_SelectedX Or PrevSelectedY<>Grid_SelectedY
								ChangeGridGadget = True
								GridGadgetText = "Name: "+MTRoom(x,y)\Name+Chr(13)+"Angle: "+MTRoomAngle(x,y)+"бу"
							EndIf
						EndIf
					EndIf
				Next
